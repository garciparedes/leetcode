...
impl Solution {
    pub fn get_maximum_gold(mut grid: Vec<Vec<i32>>) -> i32 {
        ...
        for i in 0..n as i32 {
            for j in 0..m as i32 {
                best = cmp::max(best, Self::mine(&mut grid, i, j));
            }
        }
        return best;
    }
    
    fn mine(grid: &mut Vec<Vec<i32>>, x: i32, y: i32) -> i32 {
        let (n, m) = (grid.len() as i32, grid[0].len() as i32);
        ...
        for (i, j) in vec![(-1, 0), (1, 0), (0, -1), (0, 1)] {
            best = cmp::max(best, Self::mine(grid, x + i, y + j));
        }
        grid[x as usize][y as usize] = current;
        best += current;
        return best;
    }
}
